// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "termsOfService": "http://swagger.io/terms/",
        "contact": {
            "name": "API Support",
            "url": "http://www.swagger.io/support",
            "email": "support@swagger.io"
        },
        "license": {
            "name": "Apache 2.0",
            "url": "http://www.apache.org/licenses/LICENSE-2.0.html"
        },
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/api/v1/process/crop": {
            "post": {
                "description": "Detect and crop the floorplan area from a paper document image",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "edge-detection"
                ],
                "summary": "Automatic crop floorplan from paper",
                "operationId": "cropFloorplan",
                "parameters": [
                    {
                        "description": "Image and edge detection options",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/handler.CropFloorplanRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Cropped floorplan",
                        "schema": {
                            "$ref": "#/definitions/handler.CropFloorplanResponse"
                        }
                    },
                    "400": {
                        "description": "Bad request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal server error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/v1/process/edges": {
            "post": {
                "description": "Process a floorplan image to detect edges using Canny edge detection",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "edge-detection"
                ],
                "summary": "Detect edges in a floorplan image",
                "operationId": "detectEdges",
                "parameters": [
                    {
                        "type": "file",
                        "description": "Floorplan image file",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    },
                    {
                        "type": "number",
                        "default": 1.2,
                        "description": "Blur radius for preprocessing",
                        "name": "blur_radius",
                        "in": "formData"
                    },
                    {
                        "type": "number",
                        "default": 50,
                        "description": "Canny low threshold",
                        "name": "canny_low",
                        "in": "formData"
                    },
                    {
                        "type": "number",
                        "default": 150,
                        "description": "Canny high threshold",
                        "name": "canny_high",
                        "in": "formData"
                    },
                    {
                        "type": "integer",
                        "default": 800,
                        "description": "Maximum width for resizing",
                        "name": "resize_max_width",
                        "in": "formData"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Edge detection result",
                        "schema": {
                            "$ref": "#/definitions/handler.EdgeDetectionResponse"
                        }
                    },
                    "400": {
                        "description": "Bad request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal server error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/v1/process/edges-json": {
            "post": {
                "description": "Process a floorplan image (provided as base64) to detect edges",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "edge-detection"
                ],
                "summary": "Detect edges using JSON request with base64 image",
                "operationId": "detectEdgesJSON",
                "parameters": [
                    {
                        "description": "JSON request with image and options",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Edge detection result with processed image",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal server error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/api/v1/upload": {
            "post": {
                "description": "Upload a floorplan image (PNG, JPG, JPEG) and return the detected rooms",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "upload"
                ],
                "summary": "Upload a floorplan image",
                "operationId": "uploadFloorplan",
                "parameters": [
                    {
                        "type": "file",
                        "description": "Floorplan image file",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Detection results with rooms",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal server error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "handler.CropFloorplanRequest": {
            "type": "object",
            "required": [
                "image"
            ],
            "properties": {
                "image": {
                    "description": "base64 or data:image/...",
                    "type": "string"
                },
                "options": {
                    "$ref": "#/definitions/handler.EdgeDetectionRequest"
                }
            }
        },
        "handler.CropFloorplanResponse": {
            "type": "object",
            "properties": {
                "cropped_image": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                }
            }
        },
        "handler.EdgeDetectionRequest": {
            "type": "object",
            "properties": {
                "blur_radius": {
                    "type": "number",
                    "default": 1.2
                },
                "canny_high": {
                    "type": "number",
                    "default": 150
                },
                "canny_low": {
                    "type": "number",
                    "default": 50
                },
                "resize_max_width": {
                    "type": "integer",
                    "default": 800
                }
            }
        },
        "handler.EdgeDetectionResponse": {
            "type": "object",
            "properties": {
                "message": {
                    "type": "string"
                },
                "processed_image": {
                    "description": "data:image/png;base64,...",
                    "type": "string"
                }
            }
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "localhost:8080",
	BasePath:         "/",
	Schemes:          []string{"http"},
	Title:            "FloorPlan Whiteboard API",
	Description:      "Backend API for floorplan processing and detection",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
